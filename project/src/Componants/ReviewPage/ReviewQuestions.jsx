...
const baseURL = "https://hackathoonproject-g4hkagbff0gygshm.germanywestcentral-01.azurewebsites.net/api/v1/tools/review/result";
...
export function ReviewQuestion({ questions,occupationName,SessionId })
{
    ...
    async function fetchResult() {
        ...
        const questionsAnswers = Object.entries(selectedAnswers).map(([questionId, choiceId]) => {
        ...
        try
        {
            ...
            setResult({...result,percentage:numericPercentage,
   ...
   return (
    ...
    <div className="bg-white p-6 rounded-2xl shadow-xl border border-blue-100">
        ...
        error?(<p>فشل التحميل</p>):
        !showResults ? (
            <>
            <div className="space-y-8">
                {questions.map((questionEntity) => (
                <div key={questionEntity.questionId} className="bg-blue-50 p-6 rounded-xl">
                    ...
                    {questionEntity.choices.map((choice) => (
                        <button
                        key={choice.id}
                        onClick={() => handleAnswerSelect(questionEntity.questionId, choice.id)}
                        className={`p-3 rounded-lg text-left transition-all
                            ${selectedAnswers[questionEntity.questionId] === choice.id
               ...
               <div className="flex justify-center w-full">
                    <div className="relative size-40 ">
                        <svg className="rotate-[135deg] size-full" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
                            ...
                            r="16" 
                            fill="none" 
                            className={`stroke-current  ${result.percentage>=50?'text-green-200':'text-red-500'}`}
                            strokeWidth="1"
                            strokeLinecap="round" 
                            ...
                            r="16" 
                            fill="none" 
                            className={`stroke-current  ${result.percentage>=50?' text-green-500 ':' text-red-500'} `} 
                            strokeWidth="2"
                            strokeLinecap="round"  
                            strokeDasharray={`${(result.percentage / 100) * 100.53096} 100`} 
                            ></circle>
                        </svg>

                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
                            <span className={`text-4xl font-bold ${result.percentage>=50?'text-green-600  text-green-500 ':'text-red-300 text-red-500'}`}>
                            {result.percentage}
                            </span>
                            <span className={`${result.percentage>=50?'text-green-600 text-green-500 ':'text-red-300 text-red-500'}  block`}>
                            نسبة الملائمة
                            </span>
                        </div>
                        </div>
